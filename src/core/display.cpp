...
void displayRedStripe(String text, uint16_t fgcolor, uint16_t bgcolor) {
    ...
    if (text.length() * LW * FM < (uint32_t)(tftWidth - 2 * FM * LW)) size = FM;
    else size = FP;
    tft.drawPixel(0, 0, 0);
...
Opt_Coord drawOptions(
    ...
    Opt_Coord coord;
    int menuSize = options.size();
    if (options.size() > (size_t)MAX_MENU_SIZE) { menuSize = MAX_MENU_SIZE; }
...
void jpegRender(int xpos, int ypos) {
    ...
    while (JpegDec.read()) {   // While there is more data in the file
        ...
        if (mcu_x + mcu_w <= (int)max_x) win_w = mcu_w;
        else win_w = min_w;

        // check if the image block size needs to be changed for the bottom edge
        if (mcu_y + mcu_h <= (int)max_y) win_h = mcu_h;
        else win_h = min_h;
...
bool showGif(
    ...
    do {
        ...
        if (playDurationMs > 0 && (millis() - timeStart) > (uint32_t)playDurationMs) break;
